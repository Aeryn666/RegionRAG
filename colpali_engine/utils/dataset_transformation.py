...
def load_train_set(ds_paths) -> DatasetDict:
    # ds_path = "colpali_train_set"
    # ds_path = "VisRAG-Ret-Train-In-domain-data"
    # base_path = "./data_dir/" if USE_LOCAL_DATASET else "vidore/"
    train_ds = []
    test_ds = []
    for ds_path in ds_paths:
        if len(ds_path.split(" ")) == 2:
            path, sources = ds_path.split(" ")
            if sources == '' or sources == 'all':
                sources = None
            else:
                sources = sources.split(",")
        elif len(ds_path.split(" ")) == 1:
            path = ds_path
            sources = None

        # train_ds = cast(DatasetDict, load_dataset(path))
        if 'Visual-CoT' in path:
            ds = load_visualcot_train_set(path)
        else:
            ds = load_dataset(path)
        
        if sources is not None:
            ds = ds.filter(lambda i: i['source'] in sources, num_proc=12)
        
        assert 'train' in ds

        if 'val' in ds:
            train_ds.append(clean_dataset_columns(ds['train']))
            test_ds.append(clean_dataset_columns(ds['val']))
        elif 'test' in ds:
            train_ds.append( clean_dataset_columns(ds['train']))
            test_ds.append(clean_dataset_columns(ds['test']))
        else:
            d = clean_dataset_columns(ds['train']).shuffle(seed=42)
            test_d = d.select(range(500))
            train_d = d.select(range(500, len(d)))
...
def clean_dataset_columns(dataset: Dataset):
    valid_column_names = ["image", "image_path", "query", "source", "bbox"]
    remove_column_names = [name for name in dataset.column_names if name not in valid_column_names]
    dataset = dataset.remove_columns(remove_column_names)
    # if "bbox" not in dataset.column_names:
    #     dataset = dataset.add_column("bbox", [[], ] * len(dataset))
    # if "image_path" not in dataset.column_names:
    #     dataset = dataset.add_column("image_path", [None, ] * len(dataset))
    return dataset

def load_visualcot_train_set(path) -> DatasetDict:
    img_base = path
    data_path = os.path.join(path, "viscot_363k.json")
    dataset = cast(Dataset, load_dataset("json", data_files=data_path, split="train"))

    def extract_query(conversations):
        if not conversations:
            return ""
        text = conversations[0]["value"].replace("<image>", "").strip() # 去掉 <image> 和空格
        return text.replace("Please provide the bounding box coordinate of the region that can help you answer the question better.", "").strip()
    
    def extract_bbox(image_field):
        if "###" in image_field:
            bbox_str = image_field.split("###")[-1]
            try:
    ...
    def preprocess(example):
        ...
        bbox = extract_bbox(example["image"][1])
        image_path = os.path.join(img_base, example['image'][0])
        # image = Image.open(image_path)
        return {
            "query": query,
            # "image": None,
            "image_path": image_path,
            "source": example.get("dataset"),
            "bbox": bbox
        }
